...
- Uses the RAWG public API as the system of record for game data and Firebase Authentication + Cloud Firestore for user sessions and favorites.
...
- Manage account-scoped favorites that sync after sign-in.
...
- Favorites sync: Firestore subscription + CRUD in `src/components/FavoritesSync/FavoritesSync.tsx` and `src/lib/userFavorites.ts:1`.
